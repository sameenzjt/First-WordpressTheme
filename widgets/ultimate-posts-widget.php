...
if ( !class_exists( 'WP_Widget_Ultimate_Posts' ) ) {
	...
	class WP_Widget_Ultimate_Posts extends WP_Widget {
		...
		function WP_Widget_Ultimate_Posts() {
			...
			$widget_ops = array( 'classname' => 'widget_ultimate_posts', 'description' => __( '显示带有一系列选项的帖子列表', 'sameen' ) );
			$this->WP_Widget( 'sticky-posts', __( 'Ultimate Posts', 'sameen' ), $widget_ops );
					...
					<li>
						...
						<div class="upw-content">
							...
							<?php if ( $instance['show_cats'] ) : ?>
								<p class="post-cats">
									<span class="post-cats-label"><?php _e('分类目录', 'sameen'); ?>:</span>
							...
							<?php if ( $instance['show_tags'] ) : ?>
								<p class="post-tags">
									<span class="post-tags-label"><?php _e('标签', 'sameen'); ?>:</span>
		...
		function form( $instance ) {
			...
			if ( $instance ) {
				...
				$order = 'DESC';
				$orderby = 'date';
				$morebutton_text = __('查看更多帖子', 'sameen');
			...
			<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( '标题', 'sameen' ); ?>:</label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" /></p>

			<p><label for="<?php echo $this->get_field_id( 'title_link' ); ?>"><?php _e( '标题 URL', 'sameen' ); ?>:</label>
			...
			<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( '帖子数', 'sameen' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'show_time' ); ?>"><?php _e( '显示发布时间', 'sameen' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'show_excerpt' ); ?>"><?php _e( '显示摘录', 'sameen' ); ?></label>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('excerpt_length'); ?>"><?php _e( '摘录长度（以字为单位）', 'sameen' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('excerpt_readmore'); ?>"><?php _e( '阅读更多文字', 'sameen' ); ?>:</label>
			...
			<?php if ( function_exists('the_post_thumbnail') && current_theme_supports( 'post-thumbnails' ) ) : ?>
				...
				<p>
					<label><?php _e('缩略图大小', 'sameen'); ?>:</label>
					<br />
					<label for="<?php echo $this->get_field_id('thumb_w'); ?>">
						<?php _e('W', 'sameen'); ?>: <input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id('thumb_w'); ?>" name="<?php echo $this->get_field_name('thumb_w'); ?>" value="<?php echo $thumb_w; ?>" />
					</label>
					<label for="<?php echo $this->get_field_id('thumb_h'); ?>">
						<?php _e('H', 'sameen'); ?>: <input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id('thumb_h'); ?>" name="<?php echo $this->get_field_name('thumb_h'); ?>" value="<?php echo $thumb_h; ?>" />
					</label>
				</p>

				<p>
					<label><?php _e('裁切模式', 'sameen'); ?>:</label>
					<br />
					<select id="<?php echo $this->get_field_id('thumb_crop'); ?>" name="<?php echo $this->get_field_name('thumb_crop'); ?>">
						<option value="1"<?php if($thumb_crop == 1) echo ' selected'; ?>><?php _e('Scale &amp; Crop to Fit', 'sameen'); ?></option>
						<option value="0"<?php if($thumb_crop == 0) echo ' selected'; ?>><?php _e('拉伸至适合', 'sameen'); ?></option>
						<option value="2"<?php if($thumb_crop == 2) echo ' selected'; ?>><?php _e('Proportional Scale', 'sameen'); ?></option>
						<option value="3"<?php if($thumb_crop == 3) echo ' selected'; ?>><?php _e('Proportional Scale No Borders', 'sameen'); ?></option>
					</select>
				</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_morebutton'); ?>"> <?php _e('显示更多按钮', 'sameen'); ?></label>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('morebutton_text'); ?>"><?php _e( '更多按钮文字', 'sameen' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('morebutton_url'); ?>"><?php _e( '更多按钮 URL', 'sameen' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_cats'); ?>"> <?php _e('显示帖子类别', 'sameen'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_tags'); ?>"> <?php _e('Show post tags', 'sameen'); ?></label>
			</p>

			<p>
			<label for="<?php echo $this->get_field_id('cats'); ?>"><?php _e( '选择类别', 'sameen' ); ?>:</label>
			...
			<p>
			<label for="<?php echo $this->get_field_id('types'); ?>"><?php _e( '选择帖子类型', 'sameen' ); ?>:</label>
			...
			<p>
				...
				<select name="<?php echo $this->get_field_name('orderby'); ?>" id="<?php echo $this->get_field_id('orderby'); ?>" class="widefat">
					...
					<option value="title"<?php if( $orderby == 'title') echo ' selected'; ?>><?php _e('Title', 'sameen'); ?></option>
			...
			<p class="credits"><small><?php _e('Developed by', 'sameen'); ?> <a href="http://pomelodesign.com">Pomelo Design</a></small></p>
